...
public class AsientosMatriz extends javax.swing.JFrame {
    //atributos
    AgenciaDeViajes agencia;
    Vuelo v1;
    Asiento[][] asiento;
    int [][] usuariosAsientos;
    boolean[][] confirmarAsiento;
    ...
    private void initComponents() {
        ...
        jLabel10.setText("Reservar Asientos");
        ...
        btnRandom.setIcon(new javax.swing.ImageIcon(getClass().getResource("/button (12).png"))); // NOI18N
        ...
        btnContinuar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/button (11).png"))); // NOI18N
        ...
        jPanel8Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel8Layout.createSequentialGroup()
                ...
                .addGroup(jPanel8Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel8Layout.createSequentialGroup()
                        .addGap(78, 78, 78)
                        .addComponent(btnContinuar, javax.swing.GroupLayout.PREFERRED_SIZE, 247, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(0, 0, Short.MAX_VALUE)))
                .addContainerGap())
            .addGroup(jPanel8Layout.createSequentialGroup()
                .addGap(63, 63, 63)
                .addComponent(btnRandom, javax.swing.GroupLayout.PREFERRED_SIZE, 279, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void btnRandomActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRandomActionPerformed
        ...
        int contador = 0;
        int m = 0;
        int cantidad = Integer.parseInt(JOptionPane.showInputDialog("Ingrese la cantidad de asientos: "));
        while(m<cantidad){
            //ArrayList<String> listaId = new ArrayList<String>();
            System.out.println("Valor m "+m);
            System.out.println("Vallor cantidad "+cantidad);
            ArrayList<Vuelo> listaVuelos = new ArrayList<Vuelo>();
            Usuario u1 = new Usuario();
            //Lee la información del usuario y vuelo actual
            u1 = agencia.leerUsuarioActual();
            v1 = agencia.leerVueloActual();

            while(asiento[posicionColumna][posicionFila].getEstado()==1)
            {
                contador++;
                ...
                if(contador==30){
                    JOptionPane.showMessageDialog(null, "No hay más asientos de clase economica, "
                            + "debe reservar otro por un costo adicional", "Información", JOptionPane.INFORMATION_MESSAGE);
                }
            }

            //lee la información del archivo de vuelo
            agencia.leerVuelos();
            //inicializa el indice en -1 para poder utilizarlo más adelante
            int indice = -1;

            /*este ciclo for lo que hace es buscar si el vuelo que se ha seleccionado se encuentra en los vuelos del archivo. Esto con el fin de
            conseguir el indice del vuelo*/
            for(int i=0; i<agencia.getVuelos().size(); i++){
                if(agencia.getVuelos().get(i).getIdVuelo() == v1.getIdVuelo()){
                    indice = i;
                }
            }

            //estas son todas las matrices que se deben modificar para actualizar los datos en cada vuelo
            usuariosAsientos = agencia.getVuelos().get(indice).getUsuariosAsientos();//le pasamos los datos actuales de la agencia de vuelos
            confirmarAsiento = agencia.getVuelos().get(indice).getConfirmarAsientoReserva();
            asiento[posicionColumna][posicionFila].setEstado(1);

            //este es la matriz de botones. Es solo para poder saber que boton presionó y así cambiar el color en base a la posición
            matriz[posicionColumna][posicionFila].setIcon(ocupado);

            //Aquí es donde sucede la magia
            usuariosAsientos[posicionColumna][posicionFila] = u1.getCedula();
            confirmarAsiento[posicionColumna][posicionFila] = true;

            /**********************************************************************************************************************************/
            //Este bloque de código guarda los cambios de estados realizados en los asientos del vuelo respectivo
            agencia.getVuelos().get(indice).setAsientos(asiento);
            agencia.getVuelos().get(indice).setUsuariosAsientos(usuariosAsientos);
            agencia.getVuelos().get(indice).setConfirmarAsientoReserva(confirmarAsiento);

            //lee los cambios que se realizaron en el vuelo y en sus asientos y los añade al ArrayList de Vuelos global
            agencia.añadirVuelos(agencia.getVuelos());

            JOptionPane.showMessageDialog(null,"se ha reservado el asiento de forma exitosa", "confirmación", JOptionPane.INFORMATION_MESSAGE);
            /**********************************************************************************************************************************/
            ...
            for(int i=0; i<listaVuelos.size(); i++){
                if(v1.getIdVuelo() == listaVuelos.get(i).getIdVuelo()){
                    listaVuelos.get(i).setUsuariosAsientos(usuariosAsientos);
                    listaVuelos.get(i).setAsientos(asiento);
                    listaVuelos.get(i).setConfirmarAsientoReserva(confirmarAsiento);
                    encontrado =1;
                }
            }
            //si no existe el vuelo en la lista de compra lo que hace es agregarlo como un nuevo vuelo
            if(encontrado !=1){
                v1.setAsientos(asiento);
                v1.setUsuariosAsientos(usuariosAsientos);
                v1.setConfirmarAsientoReserva(confirmarAsiento);
                listaVuelos.add(v1);
            }
            //aquí se añaden los cambios en la lista de compras de vuelos
            agencia.añadirVueloListaDeCompra(listaVuelos);
            btnContinuar.setEnabled(false);

            m++;
            if(posicionColumna == 5 && posicionFila == 15){
                m=cantidad+1;
                JOptionPane.showMessageDialog(null, "No se encuentran asientos de clase economica disponible,"
                        + " debe reservar su asiento por un costo adicional ", "Información", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    ...
    private void btnContinuarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnContinuarActionPerformed
        ...
        usuariosAsientos = agencia.getVuelos().get(indice).getUsuariosAsientos();//le pasamos los datos actuales de la agencia de vuelos
        confirmarAsiento = agencia.getVuelos().get(indice).getConfirmarAsientoReserva();
        /*posible error*/asiento[columnaAsiento][filaAsiento].setEstado(1);

        //este es la matriz de botones. Es solo para poder saber que boton presionó y así cambiar el color en base a la posición
        matriz[columnaAsiento][filaAsiento].setIcon(ocupado);

        //Aquí es donde sucede la magia
        usuariosAsientos[columnaAsiento][filaAsiento] = u1.getCedula();
        confirmarAsiento[columnaAsiento][filaAsiento] = true;
        ...
        for(int i=0; i<agencia.getVuelos().size(); i++){
            for(int j=0; j<6; j++){
                for(int k=0; k<15; k++){
                    if(agencia.getVuelos().get(indice).getConfirmarAsientoReserva()[j][k]==true){
                        contador++;
                        System.out.println("Esta es la cantidad de true: "+ contador);
                    }
                }
            }
        }
